...
export const createTask = (auth) => {
    ...
    const response = http.post(taskBaseUrl, JSON.stringify(generatedTask), auth);
...
export const updateTask = (id, auth) => {
    ...
    const response = http.put(`${taskBaseUrl}/${id}`, JSON.stringify(updatedTask), auth);
...
export const deleteTask = (id, auth) => {
     http.del(`${taskBaseUrl}/${id}`, null, auth);
}
