...
func (s *UserStore) Create(user *User) error {
	...
	if err != nil {
		log.Printf("user create failed: %v\n", err)
		return err
	}
	return nil
}
...
func (s *UserStore) UpdateToken(id int, token string) error {
	...
	if err != nil {
		log.Printf("user token update failed: %v\n", err)
		return err
	}
	return nil
}
...
func (s *UserStore) FindByEmail(email string) (*User, error) {
	...
	if err != nil {
		log.Printf("find user by email failed: %v\n", err)
		return &User{}, err
	}
	return &u, nil
}
...
func (s *UserStore) FindByToken(email string, token string) (*User, error) {
	...
	if err != nil {
		log.Printf("find user by email %d and token failed %s: %v\n", email, token, err)
		return &User{}, err
	}
	return &u, nil
}
...
func (s *UserStore) DeleteToken(id int, token string) error {
	...
	if err != nil {
		log.Printf("delete user %d token failed: %v\n", id, err)
		return err
	}
	return nil
}
...
func (s *UserStore) DeleteAllTokens(id int) error {
	...
	if err != nil {
		log.Printf("delete user %d all tokens failed: %v\n", id, err)
		return err
	}
	return nil
}
